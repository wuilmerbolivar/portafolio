...
export default function Contact() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Customize text slightly based on language for better UX
  ...
  return (
    <section id="contact" className="bg-brand-card border border-brand-green/20 rounded-2xl p-6 shadow-lg relative overflow-hidden">
      <div className="absolute inset-0 bg-brand-green/5 opacity-50 z-0 pointer-events-none"></div>
          ...
          <div className="w-full max-w-105 bg-[#0b141a] rounded-2xl overflow-hidden border border-white/10 shadow-2xl flex flex-col h-120">
            ...
            <div className="bg-[#202c33] px-3 py-3 flex items-center gap-3 shadow-md z-10 cursor-default">
              ...
              <div className="flex gap-4 text-[#aebac1] pr-2">
                <Video size={18} className="cursor-not-allowed hover:text-white transition-colors" />
                <Phone size={18} className="cursor-not-allowed hover:text-white transition-colors" />
                <MoreVertical size={18} className="cursor-not-allowed hover:text-white transition-colors" />
              </div>
            </div>

            {/* WhatsApp Body */}
            <div 
              className="flex-1 p-4 overflow-y-auto hidden-scrollbar flex flex-col gap-3 relative" 
            ...
            >
              ...
              {step >= 1 && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#202c33] text-[#e9edef] rounded-tr-lg rounded-br-lg rounded-bl-lg rounded-tl-sm p-2.5 px-3 max-w-[88%] self-start shadow-sm relative group">
                  <p className="text-[0.875rem] leading-[1.4] pr-10 pb-2">
                    {t('contact.msg1')}
                  </p>
                  <span className="text-[#8696a0] text-[0.65rem] absolute bottom-1 right-2">
                    {currentTime}
                  </span>
                </motion.div>
              )}
              ...
              {submitted && (
                <motion.div initial={{ opacity: 0, scale: 0.95, originX: 1, originY: 1 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#005c4b] text-[#e9edef] rounded-tl-lg rounded-br-sm rounded-bl-lg rounded-tr-sm p-2.5 px-3 max-w-[88%] self-end shadow-sm relative mt-auto">
                  <p className="text-[0.875rem] leading-[1.4] pr-14 pb-2 whitespace-pre-wrap break-words">{message}</p>
                  <div className="absolute bottom-1 right-2 flex items-center gap-1">
                    <span className="text-[#8696a0] text-[0.65rem] font-medium opacity-80">
                      {currentTime}
                    </span>
            ...
            <form onSubmit={handleSubmit} className="bg-[#202c33] px-2 py-2.5 flex items-end gap-2 shrink-0 z-10 w-full">
                ...
                <textarea 
                  ...
                  onKeyDown={(e) => {
                    if(e.key === 'Enter' && !e.shiftKey) {
                      e.preventDefault();
                      if(message.trim()) handleSubmit(e as unknown as FormEvent);
                    }
                  }}
                />
              </div>
              <button 
                ...
                aria-label="Enviar"
              >
                <Send size={20} className={message.trim() ? "-ml-0.5" : "-ml-0.5"} fill={message.trim() ? "currentColor" : "none"} />
              </button>
            </form>
